...
bool WrappedIsPackageOwnedPath(const std::string& lhs, const std::string& rhs) {
    auto fn = Process().originalIsPackageOwnedPath;
    if (fn == nullptr) {
        return false;
    }
...
extern "C" int WrappedStrcasecmp(const char* lhs, const char* rhs) {
    auto fn = Process().originalStrcasecmp;
    if (fn == nullptr) {
        return UnicodePolicy::CompareCaseFoldIgnoringDefaultIgnorables(
            reinterpret_cast<const uint8_t*>(lhs ? lhs : ""),
            (lhs != nullptr) ? std::strlen(lhs) : 0,
            reinterpret_cast<const uint8_t*>(rhs ? rhs : ""),
            (rhs != nullptr) ? std::strlen(rhs) : 0);
    }
...
extern "C" bool WrappedEqualsIgnoreCaseAbi(const char* lhsData, size_t lhsSize, const char* rhsData,
                                           size_t rhsSize) {
    auto fn = Process().originalEqualsIgnoreCase;
    if (fn == nullptr) {
        return UnicodePolicy::CompareCaseFoldIgnoringDefaultIgnorables(
                   reinterpret_cast<const uint8_t*>(lhsData ? lhsData : ""), lhsSize,
                   reinterpret_cast<const uint8_t*>(rhsData ? rhsData : ""), rhsSize) == 0;
    }
    const int result = UnicodePolicy::CompareCaseFoldIgnoringDefaultIgnorables(
        reinterpret_cast<const uint8_t*>(lhsData ? lhsData : ""), lhsSize,
